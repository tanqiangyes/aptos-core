...
impl NibblePath {
    ...
    pub fn new_from_state_key(state_key: &StateKey, num_nibbles: usize) -> Self {
        NibblePath::new_from_byte_array(state_key.hash().to_vec(), num_nibbles)
    }

    fn new_from_byte_array(mut bytes: Vec<u8>, num_nibbles: usize) -> Self {
        ...
        if num_nibbles % 2 == 1 {
            ...
            bytes[num_bytes - 1] = last_byte_padded;
            NibblePath {
                num_nibbles,
                bytes: bytes[..num_bytes].to_vec(),
            }
        } else {
